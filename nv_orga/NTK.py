    ...
    for i in range(X.size()[0]):
        ...
        for param in params: #param.requires_grad dans PyTorch est un attribut des tenseurs (y compris ceux qui représentent les paramètres d'un modèle, comme les poids et les biais d'un réseau de neurones) qui détermine si PyTorch doit calculer les gradients pour ces tenseurs pendant la rétropropagation.
            if param.requires_grad:
                #print(param.grad.shape, np.array(param.grad).shape, (np.array(param.grad) == 0).sum())
                #if (np.array(param.grad) == 0).sum() !=0:
                    #print(param)
...
def client_compute_eNTK(client_model, train_loader, subsample_size=100000):
    ...
    for data, target in train_loader:
        grads = compute_eNTK(client_model, data, subsample_size=subsample_size)
        grads = grads.float()
        ...
        cpt_batch += 1
        print('num batch :', cpt_batch, end='\r')
