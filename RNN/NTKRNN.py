...
def compute_eNTK(model, X, subsample_size=10000, seed=123):
    """"compute eNTK
    ...
    n_params = sum([np.prod(p.size()) for p in params]) #nombre total de paramètres trainable du modèle
...
def client_compute_eNTK(client_model, train_loader, subsample_size=10000):
    """Train a client_model on the train_loder data."""
    client_model.train()

    data, targets = next(iter(train_loader))
    grads_data = compute_eNTK(client_model, data, subsample_size) #supprimer .cuda()
...
def compute_eNTK_RNN(model, train, subsample_size=10000, seed=123):
    """"compute eNTK
    ...
    for i in tqdm(range(len(train))):
        ...
        model.zero_grad() #réinitialise les gradients
        # for i in range(train[i][0].size()[0]):
        #     output, hidden = model(train[i][0], hidden)
        # grad_i = torch.autograd.grad(outputs=output, inputs=input_sequence, grad_outputs=torch.ones_like(output))[0] #calcule les gradients  
        for j in range(train[i][0].size()[0]):
            model.forward(train[i][0][j], hidden)[0][0][0].backward() #calcule les gradients
        grad = []
        for param in params: #param.requires_grad dans PyTorch est un attribut des tenseurs (y compris ceux qui représentent les paramètres d'un modèle, comme les poids et les biais d'un réseau de neurones) qui détermine si PyTorch doit calculer les gradients pour ces tenseurs pendant la rétropropagation.
            if param.requires_grad:
                grad.append(param.grad.flatten())
        ...
        if grads is None:
            grads = torch.zeros((len(train), grad.size()[0]), dtype=torch.half) #Si grads est None, une matrice zéro de la forme appropriée est initialisée.
        grads[i, :] = grad #Stocke le vecteur de gradient subsamplé pour l'échantillon i dans la matrice grads

    return grads

def client_compute_eNTK_RNN(client_model, train, subsample_size=10000):
    """Train a client_model on the train_loder data."""
    client_model.train()
    grads_data = compute_eNTK_RNN(client_model, train, subsample_size) #supprimer .cuda()
    grads_data = grads_data.float() #supprimer .cuda()
    target_list = []
    for i in range(len(train)) :
        target_list.append(train[i][1])
